...
export default function EmissionFactorsPage() {
  ...
  const openEdit = (row) => {
    ...
    setFormValues({
      0: st.name || st.sub_category || st.factor_name || '',
      1: st.category || 'Transport',
      2: st.image ? `${API_CONFIG.IMAGE_URL}${st.image}` : '',
      3: st.unit || '',
      4: st.factor !== undefined ? String(st.factor) : (st.value !== undefined ? String(st.value) : ''),
  ...
  const saveRecord = async () => {
    try {
      ...
      formData.append('category', formValues[1])
  ...
  const columns = [
      ...
      render: (row) => {
        ...
        return (
          <div className="flex items-center gap-4 py-1">
            {row.original.image ? (
              <img src={`${API_CONFIG.IMAGE_URL}${row.original.image}`} alt={name} className="w-[36px] h-[36px] rounded-lg object-cover bg-white border border-pink-100 p-0.5 shadow-sm" />
            ) : (
    ...
    { 
      key: 'col-4', 
      label: 'STATUS', 
      render: (row) => <StatusToggle active={row.original.status !== false} tone="pink" onChange={(newStatus) => handleStatusChange(row, newStatus)} /> 
    },
    {
      key: 'col-5',
      label: 'ACTIONS',
      ...
      {modalOpen && (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Field label="Category" type="select" options={['Transport', 'Energy', 'Food', 'Waste', 'Manufacturing', 'Agriculture']} value={formValues[1]} onChange={(val) => setFormValues(c => ({...c, [1]: val}))} />
            
            <div className="md:col-span-2 flex flex-col gap-2">
              <Field label="Display Image" type="file" full onChange={(val) => setFormValues(c => ({...c, [2]: val}))} />
              {formValues[2] && (
                <div className="flex items-center gap-3 mt-1 p-2 bg-gray-50 rounded-xl border border-gray-100 w-max pr-4">
                  ...
                  <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
                    {typeof formValues[2] === 'string' ? 'Current Image' : 'New Image Selected'}
                  </span>
                </div>
              )}
            </div>

            <Field label="Unit" required placeholder="e.g. kg/km, kg/kWh" value={formValues[3]} onChange={(val) => setFormValues(c => ({...c, [3]: val}))} />
            <Field label="Factor Value" required type="number" placeholder="e.g. 0.21" value={formValues[4]} onChange={(val) => setFormValues(c => ({...c, [4]: val}))} />
