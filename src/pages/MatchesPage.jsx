...
export default function MatchesPage() {
  ...
  const fetchTeams = async (tournamentId) => {
    try {
      ...
      let teamList = res?.data?.data || res?.data || [];

      if (!Array.isArray(teamList) && teamList.docs) {
        teamList = teamList.docs;
      }
  ...
  const openEdit = (row) => {
    ...
    setFormValues({
      0:
        st.tournament_id?._id ||
        st.tournament_id ||
        (tournaments.length > 0 ? tournaments[0]._id : ""),
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex flex-col gap-4 py-2">
          <div>
            <span className="bg-pink-50 text-pink text-[9px] font-black px-2 py-1 rounded uppercase tracking-widest">
              {row.original.tournament_id?.short_name || "—"}
            </span>
          </div>
          <div className="flex items-center gap-6 max-w-max">
            <div className="flex items-center gap-3 w-[100px]">
              {row.original.team1_id?.team_logo ? (
                <img
                  src={`${API_CONFIG.IMAGE_URL}${row.original.team1_id.team_logo}`}
                  className="w-10 h-10 rounded-lg object-contain bg-white border border-gray-100 p-1"
                  alt={row.original.team1_id.team_short_name}
                />
              ) : (
                <div className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200" />
              )}
              <b className="text-[14px] font-black tracking-wider text-gray-900">
                {row.original.team1_id?.team_short_name || "T1"}
              </b>
            </div>

            <span className="text-[9px] font-black text-pink bg-pink-50 w-6 h-6 flex items-center justify-center rounded-full shadow-sm">
              VS
            </span>

            <div className="flex items-center gap-3 w-[100px]">
              {row.original.team2_id?.team_logo ? (
                <img
                  src={`${API_CONFIG.IMAGE_URL}${row.original.team2_id.team_logo}`}
                  className="w-10 h-10 rounded-lg object-contain bg-white border border-gray-100 p-1"
                  alt={row.original.team2_id.team_short_name}
                />
              ) : (
                <div className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200" />
              )}
              <b className="text-[14px] font-black tracking-wider text-gray-900">
                {row.original.team2_id?.team_short_name || "T2"}
              </b>
            </div>
          </div>
        </div>
    ...
    },
    {
      ...
      render: (row) => {
        const dateStr = row.original.match_date
          ? new Date(row.original.match_date).toLocaleDateString("en-US")
          : "—";
        return (
          <div className="flex flex-col gap-2.5 py-2">
            <strong className="flex items-center gap-2 text-[12px] font-bold text-gray-900">
              <Icon name="calendar" size={15} className="text-pink" />
              {dateStr}
            </strong>
            <span className="flex items-center gap-2 text-[12px] font-semibold text-gray-400">
              <Icon name="clock" size={15} />
              {row.original.match_time || "—"}
            </span>
            <span className="flex items-center gap-1.5 text-[10px] font-black tracking-widest uppercase text-[#0033cc]">
              <Icon name="pulse" size={14} />
              {row.original.match_status || "—"}
            </span>
            <span className="flex items-center gap-2 text-[11px] font-semibold text-gray-500 mt-1">
              <Icon name="pin" size={15} className="text-[#3da9ef]" />
              {row.original.venue || "—"}
            </span>
          </div>
        );
      ...
      render: (row) => {
        ...
        return (
          <div className="flex flex-col gap-3 items-start py-2">
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-[#f0f5ff] text-[#0033cc] rounded-lg text-[10px] font-black tracking-widest uppercase">
              <Icon name="pulse" size={14} /> TOTAL: {total}
            </span>
            <div className="flex flex-col gap-1.5 w-[100px] pl-1">
              <div className="flex justify-between items-center w-full">
                <span className="text-[9px] font-black tracking-widest text-gray-400">
                  {row.original.team1_id?.team_short_name || "T1"}
                </span>
        ...
        return (
          <div className="flex flex-col gap-3.5 py-2">
            <div className="flex items-center gap-4 w-[240px]">
              <span className="text-[10px] font-black text-[#0033cc] w-8">
                {t1_name}
              </span>
              <span className="bg-orange-50 text-orange-500 px-2 py-1 rounded text-[9px] tracking-widest uppercase font-black">
                Rem: {t1_rem}
              </span>
              <span className="bg-pink-50 text-pink px-2 py-1 rounded text-[9px] tracking-widest uppercase font-black">
                Planted: {t1_plant}
              </span>
            </div>
            <div className="flex items-center gap-4 w-[240px]">
  ...
  const team2Options = teamOptions.filter(
    (team) => team.value !== formValues[1],
  );
  return (
    <>
      <PageHeader
        title="Matches"
        description="Schedule matches and record green ball statistics"
      ...
      {modalOpen && (
        <Modal
          title={editingId ? "Edit Match" : "Register New Match"}
          submitLabel={editingId ? "SAVE CHANGES" : "REGISTER MATCH"}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Field
              label="Team 1 Dot Balls"
              type="number"
              value={formValues[8]}
              onChange={(val) => setFormValues((c) => ({ ...c, [8]: val }))}
            />
            <Field
              label="Team 2 Dot Balls"
              type="number"
              value={formValues[9]}
