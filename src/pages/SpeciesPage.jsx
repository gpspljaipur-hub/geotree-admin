...
export default function SpeciesPage() {
  ...
  const fetchSites = async () => {
    try {
      ...
      const res = await apiService.getPlantationSites({
        page: 1,
        limit: 1000,
      });
  ...
  const filteredRows = useMemo(
    () =>
      rows.filter((row) =>
        (row.original.name || row.original.species_name || "")
          .toLowerCase()
  ...
  const openEdit = (row) => {
    ...
    setFormValues({
      ...
      image: st.species_image
        ? `${API_CONFIG.IMAGE_URL}${st.species_image}`
        : "",
      status: st.status !== false ? "Active" : "Inactive",
      description: st.description || "",
      variations: st.variations || [],
      growth_rate: st.growth_rate || "High",
      maintenance: st.maintenance || "High",
      sub_type: st.sub_type || "",
      site_id: st.site_id?._id || "",
    });
    setNewVariation({ height: "", price: "" });
    setModalOpen(true);
  };
  ...
  const saveRecord = async () => {
    try {
      ...
      formData.append("growth_rate", formValues.growth_rate || "High");
      formData.append("maintenance", formValues.maintenance || "High");
      formData.append("sub_type", formValues.sub_type || "");
      // Variations array
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <EntityCell
          ...
          image={
            row.original.species_image
              ? `${API_CONFIG.IMAGE_URL}${row.original.species_image}`
              : null
          }
        />
      ),
      ...
      render: (row) => (
        <div className="flex flex-col gap-1">
          <span className="text-[12px] font-black text-blue-800 flex items-center gap-1">
            ...
            {row.original.co2_absorption || 0} kg/yr
          </span>
          <span className="text-[10px] font-bold text-gray-500 flex items-center gap-1">
            <span className="text-gray-400">⏱️</span>{" "}
            {row.original.maturity_period || "—"}
          </span>
        </div>
      ),
    },
        ...
        return (
          <div className="flex flex-col gap-1">
            {variations.map((v, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1 px-2 py-1 bg-gray-50 rounded-lg text-[10px] font-black text-gray-500 w-fit"
              >
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Field
              ...
              onChange={(val) => setFormValues((c) => ({ ...c, name: val }))}
            />
            <Field
              label="Species Type"
              required
              placeholder="e.g. Fruit , Flowering, Shade"
              value={formValues.sub_type || ""}
              onChange={(val) =>
                setFormValues((c) => ({ ...c, sub_type: val }))
              }
            />
            <Field
            ...
            <Field
              ...
              onChange={(val) =>
                setFormValues((c) => ({ ...c, scientific_name: val }))
              }
            />
            <Field
              label="Sequestration (KGS/YEAR)"
              placeholder="0"
              value={formValues.co2_absorption || ""}
              onChange={(val) =>
                setFormValues((c) => ({ ...c, co2_absorption: val }))
              }
            />
            <Field
              label="Maturity Period"
              placeholder="e.g. 10-15 Years"
              value={formValues.maturity_period || ""}
              onChange={(val) =>
                setFormValues((c) => ({ ...c, maturity_period: val }))
              }
            />
            ...
            <Field
              ...
              value={formValues.growth_rate || "High"}
              onChange={(val) =>
                setFormValues((c) => ({ ...c, growth_rate: val }))
              }
            />
            ...
            <Field
              ...
              type="select"
              options={["High", "Medium", "Low"]}
              value={formValues.maintenance || "High"}
              onChange={(val) =>
                setFormValues((c) => ({ ...c, maintenance: val }))
              }
            />

            <div className="flex flex-col gap-2 md:col-span-2">
              <Field
                label="Species Image Upload (Optional on Edit)"
                type="file"
                full
                onChange={(val) => setFormValues((c) => ({ ...c, image: val }))}
              />
              {formValues.image && (
            ...
            <div className="col-span-1 md:col-span-2">
              <label className="block text-[11px] font-[850] text-gray-700 uppercase tracking-wider mb-2">
                HEIGHT & PRICE
              </label>
              <div className="border border-gray-100 rounded-2xl p-6 bg-gray-50/50">
                {!formValues.variations ||
                formValues.variations.length === 0 ? (
                  <p className="text-[12px] font-semibold text-gray-400 italic mb-4">
                    No items added yet
                  </p>
                ) : (
                  <div className="mb-4 flex flex-wrap gap-4">
                    {formValues.variations.map((v, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-4 bg-white px-4 py-3 rounded-2xl border border-gray-200 shadow-sm w-fit"
                      >
                        <div className="flex flex-col gap-1">
                          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">
                            HEIGHT:{" "}
                            <span className="text-[12px] font-[850] text-gray-800 normal-case ml-1">
                              {v.height}
                            </span>
                          </div>
                          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">
                        ...
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center rounded-lg bg-gray-50 text-gray-500 hover:bg-gray-100 hover:text-red-500 border-0 cursor-pointer transition-colors text-sm font-bold ml-2"
                        >
                          ×
                        </button>
                      </div>
            ...
            <Field
              ...
              onChange={(val) => setFormValues((c) => ({ ...c, status: val }))}
            />
            <Field
              label="Description"
              type="textarea"
              ...
              onChange={(val) =>
                setFormValues((c) => ({ ...c, description: val }))
              }
            />
          </div>
        </Modal>
