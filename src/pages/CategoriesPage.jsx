...
export default function CategoriesPage() {
  ...
  const openEdit = (row) => {
    ...
    setFormValues({
      0: row.original.name || '',
      1: row.original.type || '',
      2: row.original.category_image ? `${API_CONFIG.IMAGE_URL}${row.original.category_image}` : '',
    ...
    { 
      ...
      render: (row) => (
        <EntityCell 
          ...
          image={row.original.category_image ? `${API_CONFIG.IMAGE_URL}${row.original.category_image}` : null}
        />
      ) 
    },
      ...
      {modalOpen && (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Field label="Category Type" required type="select" options={['Carbon', 'Occasion', 'Plantation']} value={formValues[1]} onChange={(val) => setFormValues(c => ({...c, [1]: val}))} />
            <div className="flex flex-col gap-2 md:col-span-2">
              <Field label="Category Image Upload (Optional on Edit)" type="file" full value={formValues[2]} onChange={(val) => setFormValues(c => ({...c, [2]: val}))} />
              {formValues[2] && (
