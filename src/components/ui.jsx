...
export function PageHeader({ title, description, actionLabel, onAction }) {
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
      <div>
        <h2 className="text-[28px] font-black tracking-tight text-gray-900 m-0">{title}</h2>
        <p className="text-[13px] font-bold text-gray-400 mt-2 m-0">{description}</p>
      </div>
      {actionLabel && (
        <button 
...
export function SearchBar({ placeholder = 'Search...', value, onChange, showButton = false }) {
  return (
    <div className="flex items-center gap-3 w-full max-w-[400px] h-[52px] pl-5 pr-2 py-2 bg-white border border-gray-100 rounded-[15px] shadow-[0_4px_15px_rgba(15,25,40,0.04)] mb-6 focus-within:border-[#244ea3] focus-within:shadow-[0_0_0_4px_rgba(36,78,163,0.08)] transition-all">
      <div className="text-gray-400"><Icon name="search" size={18} /></div>
      <input 
...
export function StatusToggle({ active: initialActive = true, tone = 'green' }) {
  ...
  const bgClass = active ? (isPink ? 'bg-[#fff0f7] text-[#df3b91]' : 'bg-green-50 text-green-700') : 'bg-gray-100 text-gray-500'
...
export function Actions({ onEdit, onDelete }) {
  return (
    <div className="flex items-center gap-1">
      <button className="w-8 h-8 grid place-items-center rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors cursor-pointer border-0 bg-transparent" onClick={onEdit} aria-label="Edit" type="button"><Icon name="edit" size={18} /></button>
      <button className="w-8 h-8 grid place-items-center rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors cursor-pointer border-0 bg-transparent" onClick={onDelete} aria-label="Delete" type="button"><Icon name="trash" size={18} /></button>
    </div>
  )
}
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr>
            {columns.map((column) => (
              <th key={column.key} className={`pb-4 px-4 border-b border-gray-100 text-[10px] font-bold tracking-widest text-gray-400 uppercase ${column.align === 'right' ? 'text-right' : ''}`}>
                {column.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr><td className="py-10 text-center text-sm font-medium text-gray-400" colSpan={columns.length}>{emptyText}</td></tr>
          ) : rows.map((row, index) => (
            <tr key={row.id || index} className="group border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
              {columns.map((column) => (
                <td key={column.key} className={`py-4 px-4 text-[13px] font-semibold text-gray-700 ${column.align === 'right' ? 'text-right' : ''}`}>
...
export function TableCard({ children }) {
  return <div className="bg-white rounded-[24px] p-6 shadow-[0_10px_40px_rgba(20,30,50,0.02)] border border-gray-100">{children}</div>
}

export function Modal({ title, children, onClose, submitLabel = 'SAVE', onSubmit, wide = false, buttonTone = 'blue' }) {
  const isPink = buttonTone === 'pink'
  return (
    <div className="fixed inset-0 z-50 p-4 md:p-8 grid place-items-center overflow-y-auto bg-slate-900/40 backdrop-blur-sm" role="presentation" onMouseDown={onClose}>
      <section className={`relative bg-white rounded-[24px] shadow-2xl overflow-y-auto w-full ${wide ? 'max-w-[980px]' : 'max-w-[760px]'} max-h-[calc(100vh-60px)]`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
        <header className="flex items-center justify-between min-h-[76px] px-8 border-b border-gray-100">
          <h3 className="text-[21px] font-black tracking-tight text-gray-900 m-0">{title}</h3>
          <button className="p-2 text-gray-400 hover:text-gray-700 bg-transparent border-0 cursor-pointer" onClick={onClose} aria-label="Close" type="button"><Icon name="x" size={24} /></button>
        </header>
        <form onSubmit={(event) => { event.preventDefault(); onSubmit?.() }}>
          <div className="p-8">{children}</div>
          <footer className="flex justify-end gap-3 px-8 pb-8 pt-4 border-t border-gray-50">
            <button className="min-h-[46px] px-6 text-[11px] font-bold tracking-widest uppercase text-gray-600 bg-white border border-gray-200 rounded-[13px] hover:bg-gray-50 transition-colors cursor-pointer" onClick={onClose} type="button">CANCEL</button>
            <button className={`min-h-[46px] px-8 text-[11px] font-bold tracking-widest uppercase text-white rounded-[13px] transition-colors cursor-pointer border-0 shadow-lg ${isPink ? 'bg-[#df3b91] hover:bg-[#c6287c] shadow-pink-500/20' : 'bg-blue hover:bg-indigo shadow-blue/20'}`} type="submit">{submitLabel}</button>
          </footer>
        </form>
      </section>
    </div>
...
export function Field({ label, required, type = 'text', placeholder, options, full = false, value, onChange }) {
  ...
  const inputClass = "w-full min-h-[48px] px-4 bg-white border border-gray-200 rounded-[11px] text-[14px] font-medium text-gray-900 outline-none transition-all focus:border-pink focus:ring-4 focus:ring-pink/10"

  return (
    <label className={`flex flex-col gap-2 ${full ? 'col-span-full' : ''}`}>
      <span className="text-[12px] font-[800] uppercase tracking-wider text-gray-600">{label} {required && <b className="text-pink">*</b>}</span>
      {type === 'textarea' ? (
        <textarea className={`${inputClass} py-3 min-h-[88px] resize-y`} placeholder={placeholder} {...props} />
      ) : type === 'select' ? (
